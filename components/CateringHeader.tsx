"use client";

import Link from "next/link";
import { SectionIndicator } from "./SectionIndicator";

export function CateringHeader() {
  return (
    <>
      <SectionIndicator />
      {/* Sub-context strip: reminds visitor what this section is about */}
      <div
        style={{
          background: "var(--section-accent-ink, var(--ink))",
          color: "var(--bg)",
          fontSize: 11,
          letterSpacing: "0.18em",
        }}
      >
        <div
          className="shell"
          style={{
            display: "flex",
            justifyContent: "center",
            alignItems: "center",
            height: 28,
            gap: 14,
          }}
        >
          <span style={{ opacity: 0.75 }}>
            FULL CATERING · 横浜近郊｜20名様以上のご会合・ご宴会
          </span>
        </div>
      </div>

      <header
        className="section-header"
        style={{
          borderBottom: "1px solid var(--rule)",
          background: "var(--bg)",
          position: "sticky",
          top: 0,
          zIndex: 20,
        }}
      >
        <div className="shell section-header-bar">
          <Link href="/catering" className="sh-logo">
            <div className="sh-mark">株式会社イズミ産業</div>
            <div className="sh-sub">創業昭和49年 ｜ 横浜のケータリング</div>
          </Link>

          <nav className="sh-nav" aria-label="ケータリングナビゲーション">
            <Link href="/catering/menu">お料理の例</Link>
            <Link href="/catering/guide">ご利用ガイド</Link>
          </nav>

          <a href="tel:045-333-0163" className="sh-tel">
            <span aria-hidden>☎</span>
            <span className="sh-tel-num">045-333-0163</span>
          </a>

          <Link
            href="/catering/inquiry"
            className="btn btn-accent btn-sm sh-cta"
          >
            アドバイザーに相談する
          </Link>
        </div>
      </header>
    </>
  );
}
